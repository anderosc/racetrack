    ...
    if (state.currentRace?.isStarted && state.currentRace.durationSeconds >= 0 && state.currentRace?.raceMode !== "Finish") {
        ...
        if(!timerInterval){
            timerInterval = setInterval(() =>{
                ...
                raceTimer.innerHTML = `${formatTime(state.currentRace.durationSeconds)}`; //Timer: 
                } else {
        ...
        clearInterval(timerInterval);
        timerInterval = null;
        raceTimer.innerHTML = `<div id="minutes"><p style="color: black;"> ` + 0 +` </p></div>`  + ':' + `<div id="minutes"><p style="color: black;"> ` + 0 +` </p></div>`
    }
}
